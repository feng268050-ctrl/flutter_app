...
| DNS | **systemd-resolved** | networkd feeds DNS to resolved; `/etc/resolv.conf` → `../run/systemd/resolve/resolv.conf`. LLMNR/mDNS off. **NTP:** **systemd-timesyncd** (preset-enabled; HAL Auto/Manual ↔ `timedatectl set-ntp`). **DNSSEC=no** and **DNSOverTLS=no** until first-boot lock is proven (wrong RTC → `signature-expired` breaks all names). Do **not** hand-write resolv from helpers. |
