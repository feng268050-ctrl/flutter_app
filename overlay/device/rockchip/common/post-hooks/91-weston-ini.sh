...
src="${DOCKER_ROOT:-/work/lws-hmi}/overlay/board/rockchip/common/rootfs-overlay/etc/xdg/weston/weston.ini"
if [ ! -f "$src" ]; then
	src="$TARGET_DIR/usr/libexec/hmi/weston.ini"
fi
if [ ! -f "$src" ]; then
	# Fall back to generic OS overlay synced into the SDK tree.
	sdk_dir="${SDK_DIR:-${RK_SDK_DIR:-}}"
	if [ -n "$sdk_dir" ]; then
		src="$sdk_dir/buildroot/board/rockchip/common/rootfs-overlay/etc/xdg/weston/weston.ini"
	fi
fi
if [ ! -f "$src" ]; then
