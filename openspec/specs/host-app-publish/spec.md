# Host App Publish Specification

## Purpose

Host `make publish-app` / `publish-app-only`: package/sign the Flutter app install tree and upload `tar.gz` + `.sig` + `release.json` under CDN `{artifact}/app/` (default `lws-hmi/app/`).
