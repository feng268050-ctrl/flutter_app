...
The repository SHALL keep control-board `.bin` files checked in under the Flutter App `assets/firmware/control-board/` path (declared via `pubspec.yaml` parent directory asset) consumed by release and `make build-app` bundles. At runtime the App SHALL auto-select the newest matching-HW bin from that directory.

#### Scenario: Built App contains bundled firmware asset

- **WHEN** `make build-app` (or equivalent App bundle) completes with one or more configured firmware asset bins
- **THEN** the shipped App assets SHALL include `firmware/control-board/LSW01H####S####.bin` discoverable by the home bootstrap
