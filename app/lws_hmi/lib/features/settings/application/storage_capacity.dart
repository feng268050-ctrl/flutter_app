...
/// One colored segment of the Device Information storage bar.
...
/// Aggregated storage view for the iOS-style capacity bar.
...
/// Muted segment colors (System / User data / fallback). Available is separate.
...
StorageCapacitySummary summarizeStorage(List<StorageInfo> storage) {
  ...
  for (final info in storage) {
    ...
    if (_isSystemMount(info.mountPoint)) {
      // Whole rootfs is OS-owned (used + free + reserved).
      segments.add(
        StorageBarSegment(
