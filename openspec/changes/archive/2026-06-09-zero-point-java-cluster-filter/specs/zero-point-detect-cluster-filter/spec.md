...
### Requirement: Cluster valid zero-point samples within 3px tolerance

The App SHALL provide a pure-Java reducer that accepts an ordered list of native-valid zero-point samples (`ok == true`) for a single detection round. Two samples SHALL belong to the same cluster when **both** `|offset_x₁ − offset_x₂| ≤ 3` and `|offset_y₁ − offset_y₂| ≤ 3` (axis-aligned tolerance, `CLUSTER_TOLERANCE_PX = 3`).
...
- **WHEN** a round contains valid samples at `(0, 0)` and `(15, 0)`
...
- **WHEN** the first valid sample is an outlier at `(50, 0)`, four later samples cluster at `(0, 0)` within 3px, and anchor filtering would leave only the outlier
