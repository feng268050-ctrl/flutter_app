...
- **Done:** task **4.3** `make push-app` + `make debug-app` / Custom Device on pin **3.41.9**.
  ...
  - `FLUTTER_ENGINE_RUNTIME_MODE=debug FORCE=1 make build-flutter-engine` → `prebuilt/flutter-engine/3.41.9/arm64-debug` (~97M). Makefile/`br-compile-flutter` pass runtime mode into Buildroot.
  - `make build-debug-app` + `SN=SIM-0001 make debug-app` on emulator — first app visible; no kernel/engine mismatch. `make push-app` OK earlier on USB-SSH board.
