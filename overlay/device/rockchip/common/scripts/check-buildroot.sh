...
# Upstream probes sources.buildroot.net with HEAD on the site root, which
# always returns 403 (directory listing blocked). That is not a network/GFW issue.
# Probe a real file on buildroot.net instead — the same host users use for
# release tarballs (buildroot.net/downloads/buildroot-*.tar.gz).
BR_VERSION="$(grep '^export BR2_VERSION :=' "$BUILDROOT_DIR/Makefile" \
	| awk '{print $NF}')"
BR_MIRROR_PROBE="https://buildroot.net/downloads/buildroot-${BR_VERSION}.tar.gz"
"$RK_SCRIPTS_DIR/check-network.sh" "$BR_MIRROR_PROBE" buildroot.net \
	"Please retry later or check outbound HTTPS access to buildroot.net."
