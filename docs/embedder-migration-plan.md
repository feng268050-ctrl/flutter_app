...
- E3.3 boot KPI 实测与优化(R3);对齐 [`boot-verify.sh`](../overlay/board/rockchip/rk3566_rk3568/rootfs-overlay/usr/libexec/board/boot-verify.sh)。🔲 — **已知**：Weston 在 `Output enabled` 时即清掉内核 `drm_logo`（约早于 Flutter 首 present ~1.6–2.2s）；靠 desktop-shell 背景图桥接，不等同 flutter-pi「logo 留到首帧」机制。
