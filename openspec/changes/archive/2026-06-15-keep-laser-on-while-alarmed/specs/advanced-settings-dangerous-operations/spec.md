...
The Advanced Settings page SHALL include a titled group **Dangerous Operations** containing four Switch controls in this order:
...
All four fields SHALL be stored in `t_advanced_settings`. Labels MUST be localized (EN: group **Dangerous Operations**, items as named above; ZH equivalents in `values-zh`).

Each toggle row SHALL display a secondary hint line below the title explaining operator impact. Hint strings MUST be localized in EN and ZH.
...
- **AND** all four switches reflect persisted values from `t_advanced_settings`
...
- **THEN** `keepLaserOnWhileAlarmed`, `allowWorkAfterCameraAlarm`, `allowWorkAfterGasAlarm`, and `allowWorkAfterLensContamination` MUST be false
...
| Lens L001 | A production heavy contamination episode is unresolved (pending reminder after detect, or acknowledged this boot without a level-0 / fault-cleared event) or demo-sticky L001 |
...
The four dangerous-operations boolean fields MUST be read by the Advanced Settings UI and laser guard paths only. They MUST NOT be included in Advanced Settings Modbus write payloads or remote stat / device.online snapshots.
