...
The alert SHALL use alarm code **H0034**, SHALL persist a SERIOUS row in `warn_table` when the episode starts, and SHALL use the coded-alarm passive warn pipeline (`WarnCacheManager` + `DeviceDialogHandler` + `AutoDialogQueue`).

#### Scenario: Offset outside tolerance shows dialog immediately

- **WHEN** `ZeroPointDetectCoordinator` or `EdgeDrawingDetectCoordinator` finalizes a task with valid samples and `!isWithinPositionTolerance(meanOffsetX, meanOffsetY)`
- **AND** production weld alert scope is active
- **AND** `zeroPointOffsetDetectionEnabled` is true
- **THEN** the system SHALL record a zero-point offset warn log row with code **H0034**
...
#### Scenario: H0034 participates in laser interrupt

- **WHEN** H0034 passive warn is shown while laser enable is active
- **AND** `keepLaserOnWhileAlarmed` is false
- **THEN** the app SHALL force laser enable off per `alarm-laser-interrupt`
- **AND** H0034 is NOT exempt via dangerous-operations trio bypass (unlike A001/C002/L001)
...
- **AND** MUST NOT insert H0034 warn log for that session
...
- **AND** MUST NOT insert H0034 warn log
...
When the toggle is turned OFF while an H0034 episode is still active in memory, the app MAY call fault-cleared handling to dismiss the dialog and clear runtime warn cache without deleting existing `warn_table` history rows.
...
- **AND** an H0034 runtime warn episode is active
- **THEN** the system SHALL clear pending state and close the H0034 dialog if visible
- **AND** existing H0034 warn_table rows MUST remain for history
...
- **THEN** immediate H0034 log + dialog behavior SHALL apply

## REMOVED Requirements

### Requirement: Zero point offset alert shows after laser stops in production weld modes

**Reason**: Deferred presentation removed; H0034 now shows immediately and uses coded-alarm pipeline.

**Migration**: Remove references to laser falling-edge presentation; use `ZeroPointOffsetWarnAlarm` + `DeviceDialogHandler`.
