# monitor-page-glass-cards Specification

## Purpose
TBD - created by archiving change refactor-monitor-frosted-glass-cards. Update Purpose after archive.
## Requirements
...
### Requirement: Alarm Log clear action uses FrostButtonView

The Alarm Log embedded panel clear action SHALL use `FrostButtonView` with `frostedGlassButtonVariant="primary"`, `drawableStart="@drawable/btn_icon_fixed_size"`, and the same icon spacing conventions as Settings action buttons.

#### Scenario: Alarm Log clear button styling

- **WHEN** the user views the Alarm Log panel on the Alarm Information tab
- **THEN** the clear action is a `FrostButtonView` primary control
...
- **AND** the choose-video control sits below the card at default `FrostButtonView` height
...
### Requirement: AI Vision choose-video action uses FrostButtonView

The AI Vision choose-video control SHALL use `FrostButtonView` with `frostedGlassButtonVariant="primary"`, `borderGradientCenter="top-left-bottom-right"`, and height `@dimen/frost_action_button_height`. It SHALL NOT stretch to fill remaining column height.

#### Scenario: AI Vision choose-video button size

- **WHEN** the AI Vision tab is displayed
- **THEN** the choose-video button uses default `FrostButtonView` action height (`@dimen/frost_action_button_height`)
...
### Requirement: Process Video Details delete action uses FrostButtonView secondary styling

The Process Video Details delete control SHALL use `FrostButtonView` with `frostedGlassButtonVariant="secondary"`, `drawableStart="@drawable/btn_icon_fixed_size"` (same icon resource as Alarm Log clear), and `app:drawableTint="@color/frosted_glass_button_secondary_text"` so the icon matches the destructive label color.

#### Scenario: Video details delete button

- **WHEN** the user views a process video detail screen
- **THEN** the delete action is a `FrostButtonView` secondary control
