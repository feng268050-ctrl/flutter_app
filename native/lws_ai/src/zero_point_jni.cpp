// Zero-point single-frame detect JNI. Mirrors opencv_stain_detect_jni patterns.
#include "config.h"
...
#define ZP_LOGI(...) __android_log_print(ANDROID_LOG_INFO, "ZeroPointJNI", __VA_ARGS__)
#define ZP_LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "ZeroPointJNI", __VA_ARGS__)
#else
#include <cstdio>
#define ZP_LOGI(...) std::printf(__VA_ARGS__)
#define ZP_LOGE(...) std::fprintf(stderr, __VA_ARGS__)
#endif
...
zero_point::Context* fromHandle(jlong handle) {
    return reinterpret_cast<zero_point::Context*>(handle);
}

std::string detectBgrToJson(zero_point::Context* ctx, const cv::Mat& bgr) {
    const zero_point::FrameResult result = ctx->detectBgr(bgr);
    return zero_point::frameResultToJson(result, ctx->roi().reference_zero_xy);
}
...
JNI_FN(nativeCreateOpencvZeroPointDetector)(JNIEnv* env,
                                       jclass /*clazz*/,
                                       jstring roiJsonPath,
                                       jfloat tolerancePx) {
    const std::string path = getString(env, roiJsonPath);
    if (path.empty()) {
        ZP_LOGE("nativeCreateOpencvZeroPointDetector: empty roi path\n");
        return 0;
    }
    try {
        apply_opencv_detect_red_frame_gate_near(path);
        auto* ctx = new zero_point::Context(
            path,
            tolerancePx > 0.0f ? static_cast<double>(tolerancePx) : 10.0);
        return reinterpret_cast<jlong>(ctx);
    } catch (const std::exception& ex) {
        ZP_LOGE("nativeCreateOpencvZeroPointDetector failed: %s\n", ex.what());
        return 0;
    }
}

JNIEXPORT void JNICALL
JNI_FN(nativeDestroyOpencvZeroPointDetector)(JNIEnv* /*env*/, jclass /*clazz*/, jlong handle) {
    delete fromHandle(handle);
}

JNIEXPORT void JNICALL
JNI_FN(nativeSetOpencvZeroPointDetectTargetMode)(JNIEnv* /*env*/,
                                                 jclass /*clazz*/,
                                                 jlong handle,
                                                 jint mode) {
    zero_point::Context* ctx = fromHandle(handle);
    if (!ctx) {
        return;
    }
    ctx->setDetectTargetMode(mode == 1 ? zero_point::DetectTargetMode::Line
                                       : zero_point::DetectTargetMode::Point);
}

JNIEXPORT jstring JNICALL
JNI_FN(nativeOpencvZeroPointDetectFromJpg)(JNIEnv* env,
                                    jclass /*clazz*/,
                                    jlong handle,
                                    jstring imagePath) {
    zero_point::Context* ctx = fromHandle(handle);
    if (!ctx) {
        return newString(env, zero_point::errorJson(opencv_detect::kInvalidHandle,
                                                   opencv_detect::kReasonInvalidDetectorHandle));
    }
    const std::string path = getString(env, imagePath);
    if (path.empty()) {
        return newString(env, zero_point::errorJson(opencv_detect::kInvalidInput,
                                                   opencv_detect::kReasonEmptyImagePath));
    }
    cv::Mat bgr = cv::imread(path, cv::IMREAD_COLOR);
    if (bgr.empty()) {
        return newString(env, zero_point::errorJson(opencv_detect::kIoError,
...
JNIEXPORT jstring JNICALL
JNI_FN(nativeOpencvZeroPointDetectFromRgb)(JNIEnv* env,
                                  jclass /*clazz*/,
                                  jlong handle,
                                  jobject rgba,
                                  jint width,
...
JNIEXPORT jstring JNICALL
JNI_FN(nativeOpencvZeroPointDetectFromNv12)(JNIEnv* env,
                                   jclass /*clazz*/,
                                   jlong handle,
                                   jobject nv12,
                                   jint width,
