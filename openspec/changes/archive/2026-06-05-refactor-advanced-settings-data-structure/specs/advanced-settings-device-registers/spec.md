...
- **AND** the value is persisted in `t_parameter_settings`
...
The system SHALL include registers 0x009A through 0x009F in Advanced Settings device write payloads using the same write flow as existing device settings. Persisted values MUST be read from `t_parameter_settings`.
...
- **AND** each register uses the current value from `t_parameter_settings` or the default value when no persisted value exists
...
- **AND** all register values are sourced from `t_parameter_settings`
