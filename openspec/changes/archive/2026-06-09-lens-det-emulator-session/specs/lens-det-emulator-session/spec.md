...
- **AND** subsequent `nativeOpencvStainDetectFromI420(ldHandle, ...)` MUST use the `lens_det:` section from that config

#### Scenario: Invalid session handle rejected

- **WHEN** `nativeOpencvStainDetectFromI420` is called with `ldHandle == 0`
...
- **THEN** the app MUST call `inferLensDetFromI420` with source `process_video_lens_det`
