...
- Add **`lws-hmi-settings-restore.service`** oneshot (**`After=hmi.service`**, Nice/idle) that reuses existing `*-up.sh` / eth helpers; Demo watches `*-wanted` and shows starting/connecting like a manual enable while restore runs.
...
- `linux-settings-persist`: Hardware preference schema under `/var/lib/lws-hmi/`, wanted markers, boot restore oneshot, and the invariant that settings daemons must not join the HMI cgroup.

### Modified Capabilities

- `linux-wifi`: Radio enable starts `lws-hmi-wpa.service` (not in-process `wpa_supplicant -B`); DHCP via `lws-hmi-wlan0-dhcp.service`; `wifi-wanted` on enable.
- `linux-ethernet`: Apply path escapes HMI cgroup via `lws-hmi-eth0.service`; `eth0-wanted` on apply.
- `linux-backlight`: Persist brightness percent under `/var/lib/lws-hmi/backlight-brightness`.
...
- **Rootfs overlay:** new units (`lws-hmi-wpa`, `lws-hmi-wlan0-dhcp`, `lws-hmi-eth0`, `lws-hmi-settings-restore`), helpers, preset, verify scripts.
