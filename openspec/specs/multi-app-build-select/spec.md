...
## Purpose
Host Make/scripts select which Flutter app under app/ to build, push, and include in rootfs; `*_hmi` installs to /opt/hmi; optional os_settings auto-include on build-rootfs. **Product differentiation is `APP=` only** — not chip or motherboard selection at build time (generic OS model). **Kernel:** one product-line **Image** + per-`board_id` DTB in FIT (platform W5); new boards add DTS/OEM, not a separate Image build — see repo `docs/make-commands.md` §构建模型.
...
- **THEN** the build MUST use `app/lws_hmi` and install the release bundle under `app/lws_hmi/build/bundle/release/` (device path `/opt/hmi`)
...
- **THEN** the build MUST assemble only that project into `app/os_settings/build/bundle/release/`
- **AND** MUST NOT wipe or rebuild `app/lws_hmi/build/bundle/release/` when building os_settings alone
...
- **THEN** the build MUST install the release bundle under `app/cnc_hmi/build/bundle/release/` (baked to `/opt/hmi` on rootfs pack)
...
- **THEN** device `/opt/os_settings` layout MUST contain `lib/libapp.so` and `data/flutter_assets` after push or rootfs bake
- **AND** MUST NOT be required to contain `bin/mediamtx` or `bin/lws_ai_daemon`

### Requirement: push-app deploys only the selected APP

`make push-app` SHALL stream the selected app’s release bundle tree (`app/<APP>/build/bundle/release/`) over SSH and install to `/opt/hmi` for `*_hmi` (then restart `hmi.service`), or to `/opt/<APP>` for any other `APP` without restarting `hmi.service`. This is unsigned debug hot-swap (`host-push-hmi`), not a Make alias of `make upgrade-app`.
...
Before packing rootfs, `make build-rootfs` SHALL ensure the selected `APP` release bundle exists (`app/<APP>/build/bundle/release/lib/libapp.so`), copy bundles into the SDK staging overlay, and pack rootfs. When the selected APP is an HMI (`*_hmi`), the baked tree MUST be `/opt/hmi`. When `app/os_settings` exists with `pubspec.yaml`, `make build-rootfs` SHALL also ensure `/opt/os_settings` is present (building it if missing) without requiring the operator to set `APP=os_settings`. Explicit `APP=os_settings` for `build-app` / `push-app` remains required to build or push only that app interactively. **`build-rootfs` MUST NOT require chip or motherboard selection.**
...
- **THEN** the resulting rootfs staging MUST contain both `/opt/hmi/lib/libapp.so` and `/opt/os_settings/lib/libapp.so` (building missing bundles first)
...
In addition to selecting the Flutter project for `make build-app` / `push-app` / `build-rootfs`, Make/env **`APP`** SHALL select the cloud OTA publish identity for **`make publish`** / **`make publish-only`**: the R2 static-upload artifact prefix SHALL be the `APP` directory name with underscores replaced by hyphens (default `lws_hmi` → `lws-hmi`). Invalid or missing `app/<APP>/` MUST fail before upload. Non-HMI apps MUST NOT be published via the whole-device OTA publish targets unless an explicitly documented escape hatch is used.
