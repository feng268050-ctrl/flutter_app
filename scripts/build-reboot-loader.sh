#!/usr/bin/env bash
# Cross-compile reboot-loader → prebuilt/ + rootfs overlay (board libexec).
set -euo pipefail
...
sync_overlay() {
	...
	echo "build-reboot-loader: synced → $OVERLAY_BIN"
}

do_build() {
	local cc="$1"
	mkdir -p "$OUT_DIR"
	echo "build-reboot-loader: $cc (static) → $OUT_BIN"
...
if prebuilt_ready "$OUT_DIR" && [[ -x "$OUT_BIN" && -x "$OVERLAY_BIN" ]] && [[ "$FORCE" != "1" ]]; then
	echo "build-reboot-loader: prebuilt ready at $OUT_DIR"
	sync_overlay
	exit 0
fi
