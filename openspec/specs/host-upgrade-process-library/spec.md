# host-upgrade-process-library Specification

## Purpose

Host Make/SSH helper `make upgrade-process-library`: auto-select a process-library package from the device product.ini `model`, upload it, and force App import via `/run/hmi` command watcher (no version gate).
## Requirements
