...
- Full-system `make upgrade` does **not** send `factory.img`; it **streams** `rootfs.img` and the inactive letter’s FIT into partitions (helpers only under `/userdata/ota/`), and by default also streams resolved `oem.img` when present (`OEM_IMG=` empty to skip). Starts board `arm-reboot` and returns immediately (does not wait for SSH drop or post-reboot health). Online OTA stays download-then-staged-apply via `ab-upgrade-apply.sh`.
- **OEM-only iteration:** when changing only `oem/**` (board helpers, profile, screen pack, `product.ini` seed), run `make build-oem` then **`make upgrade OEM_ONLY=1`**. This streams `oem.img` into `PARTLABEL=oem` and does a **plain reboot** (no A/B letter switch, no boot/rootfs write). Prefer this over a full `make upgrade` during OEM debug.
...
| `oem/**`, `scripts/build-oem.sh`, `scripts/factory-sku.sh`, `board/factory-skus.tsv` | `make build-oem`, then **`make upgrade OEM_ONLY=1`** (oem partition only); use full `make upgrade` / `make build-img` + `make flash` only when also shipping OS or factory |
| `overlay/.../oem-compose*`, `/usr/share/hmi/oem-fallback/**` | `make apply-overlay`, `make build-rootfs`, `make upgrade` |
...
| `scripts/upgrade-remote.sh`, `scripts/stream-file-progress.py`, or Makefile `upgrade` only (board already has P2.4 overlay + A/B GPT) | `make upgrade` (or `make upgrade OEM_ONLY=1`); no firmware rebuild unless image inputs are stale |
