#!/bin/sh
# Read HUK-wrapped OP-TEE seal KEK blob from Vendor Storage (ID 23).
# Opaque wrap ciphertext only — never plaintext KEK.
#
# Usage:
#   read-seal-kek-wrapped.sh              # write raw bytes to stdout
#   read-seal-kek-wrapped.sh -o PATH      # write raw bytes to PATH
#   read-seal-kek-wrapped.sh --present    # exit 0 if blob present, 1 if absent
#
...
die() { echo "read-seal-kek-wrapped: ERROR: $*" >&2; exit 1; }
...
blob_present() {
	...
	tmp="$(mktemp)"
	if ! "$VENDOR_STORAGE_BIN" -r "$VENDOR_SEAL_KEK_WRAPPED_NAME" -t file -i "$tmp" >/dev/null 2>&1; then
		rm -f "$tmp"
		return 1
	fi
	if [ ! -s "$tmp" ]; then
...
read_blob_to() {
	...
	[ -x "$VENDOR_STORAGE_BIN" ] || return 1
	[ -e /dev/vendor_storage ] || return 1
	"$VENDOR_STORAGE_BIN" -r "$VENDOR_SEAL_KEK_WRAPPED_NAME" -t file -i "$dest" >/dev/null 2>&1
}
