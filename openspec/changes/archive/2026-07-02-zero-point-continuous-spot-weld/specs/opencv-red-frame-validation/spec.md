...
**Reason**: Requirement text implied identical color/overexposure rejection for all three modules. Zero_point adopts simplified mask-only gate; edgedrawing and lens_det behavior is unchanged in this change but no longer coupled to the same reject rules in this spec block.

**Migration**: Zero_point gate behavior is defined in the modified validator requirement above. EdgeDrawing and stain detect continue invoking `validateRedFrame` with module-appropriate semantics documented in their own capability specs when updated.
