...
- Persist sync mode + timezone under `/var/lib/lws-hmi/` so Demo choices survive HMI restart (full boot-stack restore of other hardware prefs remains **P2.3**).
...
- **Rootfs / overlay**: optional tighten of `wlan0-time-sync.sh` as the shell backend for network sync; `tzdata` / `hwclock` availability; prefs under `/var/lib/lws-hmi/` (e.g. `time-sync-mode`, `timezone`).
