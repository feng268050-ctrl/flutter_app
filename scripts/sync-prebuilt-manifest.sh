...
FLUTTER_SDK="$(read_stamp "$ROOT/prebuilt/flutter-sdk")"
...
python3 - "$MANIFEST" "$MEDIAMTX" "$RKNN_RT" "$FLUTTER_SDK" "$FLUTTER_ENGINE" "$FLUTTER_PI" <<'PY'
import json, sys
path, mediamtx, rknn, sdk, engine, pi = sys.argv[1:7]
def v(s):
