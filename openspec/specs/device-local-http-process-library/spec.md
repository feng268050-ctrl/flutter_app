...
Define LAN HTTP routes on the embedded device server (`device-local-http-api`) for remote management of engineer-mode process library entries: list, CRUD on `t_process_parameters_data`, and set active preset. Responses use `ApiResult`; paths and bodies use camelCase.
...
- **WHEN** `:id` refers to `dataType` **`1`**（工程师模式内置参数）
