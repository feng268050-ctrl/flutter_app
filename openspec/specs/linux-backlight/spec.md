...
Setting backlight brightness (from the HMI Demo or from the board shell) SHALL go through `change-backlight` / `change-backlight.sh`, which MUST apply sysfs brightness and persist the clamped percent (0–100) to `/var/lib/hmi/backlight-brightness` so boot restore can re-apply it. The Linux Flutter backlight backend MUST NOT write that preference file directly.
