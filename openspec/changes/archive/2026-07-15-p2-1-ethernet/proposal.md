...
- Add overlay helpers **`eth0-dhcp.sh` / `eth0-static.sh`** (and minimal link helpers if needed); persist eth0 IPv4 preference under `/var/lib/network/eth0-ipv4`. Still **no** `dhcpcd.service` / networkd at boot.
...
- **Overlay**: new scripts under `rootfs-overlay/usr/libexec/hmi/`; default store file under `/var/lib/hmi/`; update `verify-rootfs-overlay.sh` / `env-verify.sh` lists.
