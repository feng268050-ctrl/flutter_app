# upgrade-package-input Specification

## Purpose

`UPGRADE_PACKAGE=` for `make upgrade`: consume a prebuilt OTA `.tar` / `.tar.gz` / `.tgz` (SSH: host HTTP + sibling `.sig`; RockUSB: host extract + `di`).
