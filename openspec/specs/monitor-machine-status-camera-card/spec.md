...
Show industrial camera communication health on **Monitor → Machine Status** using the same card/checkbox pattern as Modbus machine status tiles, driven by camera ping reachability.
## Requirements
### Requirement: Machine Status shows Camera connectivity card

On **Monitor → Machine Status**, the status card grid SHALL include a **Camera** tile using the same visual pattern as existing machine status cards (label + disabled checkbox indicator inside a `FrostedGlassCard` container). The checkbox **checked** state SHALL indicate camera communication is **healthy** (ping health reports reachable). The checkbox **unchecked** state SHALL indicate camera communication is **fault** (ping health reports unreachable).
