...
- [ ] 1.1 Create `rootfs-overlay/usr/libexec/board/` and move design D1 scripts (`read-product-identity.sh`, `write-product-identity.sh`, `vendor-storage-ids.txt`, `read-device-serial.sh`, `secrets-seal` / CA helper if present under hmi)
- [ ] 1.2 Update internal defaults inside moved scripts (e.g. `VENDOR_STORAGE_IDS` path, calls between read/write helpers) to `/usr/libexec/board/…`
...
- [ ] 3.1 Update `AGENTS.md` libexec convention to include `/usr/libexec/board/` (alongside `wpa,network,bluetooth,hmi`)
- [ ] 3.2 Patch any README / `docs/storage-layout.md` lines that place identity helpers under `hmi/`
- [ ] 3.3 Align active `vendor-storage-product-identity` artifacts (or post-archive notes) so helper paths say `/usr/libexec/board/`

## 4. Verification

- [ ] 4.1 Confirm `readlink` of `/usr/bin/read-serial`, `read-identity`, `write-identity` → `/usr/libexec/board/…` on a built rootfs or overlay staging
