...
- **Board SKUs (current line):** ynh960 / ynh961 / ynh962 — same **generic OS** (shared kernel FIT + shared rootfs userspace). **Product** = `APP=`; **hardware** = OEM + FIT DTB inventory. **Validate on ynh960**; no per-SKU rootfs/kernel fork. New boards/screens → OEM packs + HAL, not `CHIP=` at build time.
...
- **Device tree / kernel fragments (until `linux-sdk` is committed):** git source of truth is **`overlay/kernel/`** (full board trees under **`overlay/kernel/rockchip/`**: `ynh960.dts`, `customer_board_ynh960.dtsi`, …), not the local SDK tree and **not** `oem/`. After editing DTS or `*.config` fragments: `make apply-overlay` then `make build-kernel`. Kernel **patches** or SDK `mk-*` script patches: **`FORCE_PLATFORM_OVERLAY=1 make apply-overlay`** (or `make squash-linux-sdk-platform`) — see pipeline rule below. Boot DTBs stay in the FIT; OEM only has runtime LCD params / profile. Detail: [`docs/linux-sdk-vendor-import.md`](docs/linux-sdk-vendor-import.md).
...
- **Generic OS vs product vs hardware (do not conflate):**
  - **One shared platform OS** — same kernel **Image** (one universal build for the product line), same Buildroot userspace stack, same `boot.img` / `boot_b.img` for all factory SKUs on that line. **`make build-kernel` does not take `APP=`**; FIT carries **multiple board DTBs** from `board/rk356x-fit-boards.txt` (W5 multi-conf), not one kernel Image per motherboard.
  - **ARM / DT model** — **Device Tree** describes board wiring and probe; **Image** holds compiled drivers/Kconfig. **New board** → new DTB + OEM (+ optional `uboot_id`); **reuse Image**. **New SoC drivers** (e.g. **ek3562** / RK3562 alongside RK3566) → extend `overlay/kernel/**/*.config`, **`FORCE_KERNEL_IMAGE=1 make build-kernel` once** for a **new universal Image**, then add DTBs to FIT inventory — **not** a per-board Image target. Runtime board_id: `/run/hmi/board_id` (`read-board-id`). Canonical: [`docs/make-commands.md`](docs/make-commands.md) → **构建模型** → Kernel / 新板接入.
  - **Product differentiation = `APP=` only** — `make build-rootfs` / `make build-app` bake **`/opt/hmi`** from the selected `*_hmi` (or `/opt/os_settings`). Artifacts: `output/firmware/<APP>/rootfs.img`. **`APP=cnc_hmi` vs `lws_hmi` = different product rootfs, same OS base.**
  - **Hardware / panel / LCD = OEM + runtime**, not a separate rootfs fork per motherboard. New board on the same platform line → FIT inventory + `oem/**` pack + `make build-oem`; **not** `CHIP=` at `build-rootfs` time.
  - **`BOARD` / `CHIP` / `DEFCONFIG`** — Rockchip **`make lunch`** SDK profile only（当前线 `CHIP=rk3566_rk3568` + `ynh960_defconfig`）。脚本经 `scripts/platform-paths.sh` 解析 SDK `buildroot/board/rockchip/common/`（通用 OS overlay + App `opt/`）与 `$CHIP/`（SoC 薄层 + lunch）。**不是** `build-kernel` / `build-rootfs` 的日常参数。Docker：repo → `/work/lws-hmi`，SDK → `/work/sdk`。
  - **New SoC on same product line** — board DTS + display params + **self-built or vendor** U-Boot/MiniLoader into repo (`overlay/kernel/`, `board/`, `prebuilt/bootloader/`, `oem/`); **no second `linux-sdk`**. U-Boot: Rockchip `u-boot` + `rkbin`/`boot_merger` ([`docs/uboot-rkbin.md`](docs/uboot-rkbin.md)). Merge SoC drivers into the **same universal Image** Kconfig; add `board_id` to FIT inventory. Rootfs userspace recipe stays shared; products stay **`APP=`** unless userspace ABI diverges. Detail: [`docs/make-commands.md`](docs/make-commands.md) → **新板 / 新 SoC 接入**.
- `make build-app` compiles release AOT into **`app/<APP>/build/bundle/release/`** only; it does **not** sync into the SDK or rebuild rootfs. Default `APP=lws_hmi`. **Convention:** Flutter dirs ending in `_hmi` install to `/opt/hmi` (for `hmi.service`); one rootfs has at most one HMI app plus optional `os_settings` at `/opt/os_settings`.
...
- **Overlay before the artifact that consumes it.** `apply-overlay` is the only step that copies overlay into the SDK. `build-kernel` / `build-rootfs` / `build-oem` never do it. If that artifact has overlay inputs, run `make apply-overlay` **first** — no exceptions for “only logo” / “only DTS”. Today: kernel ← `overlay/kernel/` + `board/logo`; rootfs ← `overlay/board/**/rootfs-overlay`; OEM packs from `oem/` (no overlay → `build-oem` does not need apply). macOS `docker-run` defaults to `SKIP_OVERLAY=1`.
- **`FORCE_PLATFORM_OVERLAY=1`:** owned SDK (`.lws-owned-tree`, normal after trim) **skips** kernel C patches and SDK `mk-*` script patches unless this is set. **Prompt for it** when changing:
  - `overlay/kernel/patches/**`
  - `overlay/device/rockchip/common/scripts/patch-mk-kernel.sh`, `patch-mk-loader.sh`, `patch-mk-rootfs.sh`, `patch-30-rootfs.sh`, `patch-post-wifibt.sh`
  Equivalent: `make squash-linux-sdk-platform`. **Do not** set it for DTS/`*.dtsi`, `overlay/kernel/**/*.config`, `overlay/kernel/firmware/`, `board/logo/`, or rootfs overlay — those sync on every plain `make apply-overlay`.
- `make build-rootfs` ensures the selected `APP` (and auto `os_settings` when present) exist under **`app/*/build/bundle/release`**, copies bundles into the **SDK Buildroot staging tree** (`linux-sdk/buildroot/board/rockchip/common/rootfs-overlay/opt/`), then packs rootfs. Selecting `APP=cnc_hmi` (etc.) replaces baked `/opt/hmi` for that product.
...
- **`make build-kernel` = Image (optional) + DTB/FIT (always).** `scripts/build-kernel-ab.sh` runs two phases:
  ...
  2. **Per-slot DTB + FIT** — always runs: force-rebuild inventory DTBs (bootargs `PARTLABEL=rootfs_a|b`), stage DTBs, patch per-slot `resource.img` PARTLABEL **and refresh RSCE ENTR SHA-1** (`scripts/patch-resource-img-partlabel.py`), repack `boot.img` / `boot_b.img`, then `--verify` RSCE hashes. A+B slots run **in parallel**; either failure kills the other. Targets: `make build-kernel` (both), `make build-kernel-a`, `make build-kernel-b`. Stale RSCE hash = B-only splash/jank/cold-boot panic — see [`docs/ab-slot-misc.md`](docs/ab-slot-misc.md).
...
- **Rule of thumb:** always `make apply-overlay` before `make build-kernel`. Add **`FORCE_PLATFORM_OVERLAY=1`** only for `overlay/kernel/patches/**` or `overlay/device/**/patch-mk-*.sh` (owned tree otherwise skips those). DTS / FIT inventory → then plain `make build-kernel`; Kconfig / drivers / logo / modules → `FORCE_KERNEL_IMAGE=1 make build-kernel`. Git SoT for kernel/DTS: `overlay/kernel/` (not OEM).
- `make build-rootfs` bakes **generic OS userspace** (platform fs-overlay: systemd, helpers, Weston stack) plus **`APP=`** App trees into rootfs and publishes `output/firmware/<APP>/rootfs.img` (boot FITs remain shared under `output/firmware/`). **Not** forked per motherboard SKU — hardware variance is **OEM**. Packs **ext2 only** (cpio/squashfs/tar unset in `chips/lws_hmi_rootfs.config`); after changing those Kconfig bits run `make apply-overlay` + `make lunch` before the next rootfs.
...
| `scripts/platform-paths.sh`, `Makefile` `CHIP`/`export` | none for firmware; scripts pick up `CHIP` (default `rk3566_rk3568`) automatically |
| `scripts/ensure-rootfs-apps.sh`, `scripts/sync-opt-app-overlay.sh` | `make build-rootfs` (runs ensure automatically; bundles must exist under `app/*/build/bundle/release` — run `make build-app` first if stale) |
| `scripts/apply-overlay.sh` (`purge_stale_fs_overlay_opt`) | `make apply-overlay`, then `make build-rootfs` to re-stage SDK `opt/` from bundles (apply-overlay never builds apps) |
...
| `board/logo/**` | `make build-boot-logo`, `make apply-overlay`, **`FORCE_KERNEL_IMAGE=1 make build-kernel`**, `make build-rootfs`, `make upgrade` |
| `board/rk356x-fit-boards.txt`, `board/boot-multi.its`, `scripts/generate-boot-fit-its.sh`, `scripts/pack-boot-fit-multi.sh`, `scripts/verify-boot-fit.sh`, `scripts/build-kernel-ab.sh`, `scripts/patch-resource-img-partlabel.py` (W5 multi-DT FIT + per-slot resource PARTLABEL/RSCE SHA-1) | `make apply-overlay`, `make build-kernel` (or `make build-kernel-a` / `make build-kernel-b`), `make upgrade` |
| `overlay/kernel/rockchip/**` DTS / `.dtsi` only (no `*.config` fragment change) | `make apply-overlay`, then `make build-kernel`, `make upgrade` |
| `overlay/kernel/drivers/net/wireless/aic8800/**`, `overlay/kernel/drivers/gpu/drm/panel/panel-simple.c` | `make apply-overlay`, then **`FORCE_KERNEL_IMAGE=1 make build-kernel`**, `make upgrade` |
| `overlay/kernel/**/*.config` (Kconfig fragments) | `make apply-overlay`, then **`FORCE_KERNEL_IMAGE=1 make build-kernel`**, `make upgrade` |
| `overlay/kernel/rockchip/ek3562-wifibt.config` (RTW88_8821CU onboard USB) | same as `*.config` fragments above; pairs with `lws_hmi_wifi_rtw88.config` on rootfs |
| `overlay/kernel/patches/**` or `overlay/device/rockchip/common/scripts/patch-mk-*.sh` / `patch-30-rootfs.sh` / `patch-post-wifibt.sh` | **`FORCE_PLATFORM_OVERLAY=1 make apply-overlay`** (or `make squash-linux-sdk-platform`), then **`FORCE_KERNEL_IMAGE=1 make build-kernel`** (and `make build-rootfs` if mk-rootfs / post-wifibt), `make upgrade` |
...
| `overlay/buildroot/chips/lws_hmi_network.config` (or other BR Kconfig that changes how an **existing** package is built, e.g. `BR2_PACKAGE_WPA_SUPPLICANT_DBUS`) | `make apply-overlay`, `bash scripts/br-make-packages.sh <label> <pkg>…`, `make check-prebuilt`, `make build-rootfs`, `make upgrade` — **not** kernel; `build-rootfs` alone will keep the old binary |
| `overlay/buildroot/chips/lws_hmi_wifi_rtw88.config` (rtw88 firmware for ek3562 onboard USB Wi‑Fi) | `make apply-overlay`, `bash scripts/br-make-packages.sh wifi linux-firmware`, `make build-rootfs`, `make upgrade` — **not** rootfs alone (new BR package) |
...
| `packages/cyber_capture` / App capture watcher（`*_hmi` 或 `os_settings`） | `make build-app` / `APP=os_settings make build-app`，再 `push-app`（native `.so` + patched `flutter-wayland-client` 需已在 rootfs） |
...
| `board/*.txt` LCD/MIPI tables | panel DT: `bash scripts/gen-ynh960-panel-init-dtsi.sh`, `make apply-overlay`, `make build-kernel` — **not** copied to rootfs `/system/etc` |
| `oem/**`, `scripts/build-oem.sh`, `scripts/factory-sku.sh`, `board/factory-skus.tsv` | `make build-oem`, then **`OEM_ONLY=1 make upgrade`** (oem partition only); new SKU e.g. `FACTORY_SKU=ek3562-dev make build-oem` (needs U-Boot blobs before `build-img`); use full `make upgrade` / `make build-img` + `make flash` only when also shipping OS or factory |
| `board/virt/**`, `scripts/build-emulator.sh`, `scripts/run-emulator.sh`, `scripts/setup-emulator-qemu.sh`, `scripts/fetch-emulator-swgl.sh`, `scripts/emulator-devices.sh`, `overlay/.../20-emulator-*.link`, `overlay/kernel/**/emulator-virtio.config`, `overlay/.../provision-mount.sh`, `overlay/.../emulator-storage-init.*` | macOS: `make setup-emulator-qemu`, `make fetch-emulator-swgl` (guest Mesa via 9p, not rootfs); virtio-sound kernel fragment → `make apply-overlay`, **`FORCE_KERNEL_IMAGE=1 make build-kernel`**, `make build-rootfs`, `make build-emulator`, `make emulator`; see `docs/p32-emulator.md` |
| `oem/**` sim-virt only | included by `make build-emulator`; or `OEM_ID=sim-virt make build-oem` |
| `overlay/.../oem-compose*` | `make apply-overlay`, `make build-rootfs`, `make upgrade` |
| `oem/packs/**/input_defaults.json`, `scripts/build-oem.sh` (pack input defaults) | `make build-oem`, then **`OEM_ONLY=1 make upgrade`** (or full `make upgrade` if also shipping rootfs); fresh devices without `input.conf` pick up seed on next compose |
| `overlay/.../apply-physical-input-policy.sh`, `weston-hmi-config.sh`, `hmi-launch.sh`, `apply-mouse-settings.sh`, `bind-prefs.sh` | `make apply-overlay`, `make build-rootfs`, `make upgrade` |
| `packages/cyber_hal` physical input policy + `app/os_settings` Keyboard/Mouse toggles | `APP=os_settings make build-app`, `APP=os_settings make push-app`; rootfs path also `make build-app`, `make push-app` for HMI bootstrap gate |
...
| `overlay/kernel/**/ynh960-optee.dtsi` + `chips/lws_hmi_optee.config` / `tee-supplicant.service` / `secrets-seal` | `make apply-overlay`, `bash scripts/br-make-packages.sh optee optee-client`, `make build-kernel`, `make build-rootfs`, `make upgrade` |
| `overlay/kernel/**/ynh960-selinux.config` + `chips/lws_hmi_selinux.config` (SELinux permissive + auditd) | `make apply-overlay`, `bash scripts/br-make-packages.sh selinux libselinux libsepol refpolicy policycoreutils libsemanage audit systemd`, **`FORCE_KERNEL_IMAGE=1 make build-kernel`**, `make build-rootfs`, `make upgrade` — SELinux does **not** require a U-Boot rebuild; force-rebuild AIC `.ko` if `CONFIG_SECURITY` just flipped (see [`docs/selinux.md`](docs/selinux.md) Wi‑Fi note) |
| Appliance Lynis hardening (`etc/modprobe.d/99-appliance-hardening.conf`, `etc/sysctl.d/99-appliance-hardening.conf`, `resolved.conf.d/10-appliance.conf`, `etc/audit/*`, `ynh960-hardening.config`) | `make apply-overlay`, `bash scripts/br-make-packages.sh selinux audit` (first auditd ship only), **`FORCE_KERNEL_IMAGE=1 make build-kernel`**, `make build-rootfs`, `make upgrade` |
...
| `overlay/buildroot/chips/lws_hmi_platform.config` / `lws_hmi_platform_prebuilt.config` (`BR2_PACKAGE_LIBGPIOD` / `_TOOLS` — `gpioset`/`gpioinfo`/`gpioget`) | `make apply-overlay`, `bash scripts/br-make-packages.sh gpiod libgpiod`, `make build-rootfs`, `make upgrade` — **not** rootfs alone (stamp reuse / prebuilt fragment must list TOOLS) |
...
- **Mandatory review gate** — every change, including documentation-only changes, must receive a review before it is handed off. After implementation and verification, inspect the final diff for correctness, regressions, localization/accessibility or UI-layout impact where applicable, and unintended file changes. The final handoff must include a concise `Review` section that lists reviewed files, findings (or “no blocking findings”), remaining risks, and verification results. Do not describe a change as complete before this review is performed. When the user requests a GitHub/PR review, additionally create or update the reviewable PR as directed.
...
- **Paths:** app → `app/<APP>/`（release bundle → `app/<APP>/build/bundle/release/`）；platform rootfs overlay → `overlay/board/rockchip/common/rootfs-overlay/`（**不含** App bundle）；SoC 薄层 → `overlay/board/rockchip/$CHIP/rootfs-overlay/`（当前线 `rk3566_rk3568`，可为空）；Buildroot fragments → `overlay/buildroot/`。
- **Do not** flash an **unvalidated** U-Boot/loader pair onto a shipping SKU. Self-building from [rockchip-linux/u-boot](https://github.com/rockchip-linux/u-boot) + [rkbin](https://github.com/rockchip-linux/rkbin) (`boot_merger`) is expected for new boards (e.g. ek3562); land artifacts under `prebuilt/bootloader/<uboot_id>/`. See [`docs/uboot-rkbin.md`](docs/uboot-rkbin.md).
...
| Path | Role |
|------|------|
| `app/lws_hmi/` | Flutter HMI → `app/lws_hmi/build/bundle/release`（进 rootfs 为 `/opt/hmi`；含产品 `bin/`，如 mediamtx）；HMI Settings = 简化+定制子集 |
...
| `packages/cyber_capture/` | Flutter seat 截屏/录屏（HMI + OS Settings；present-hook + GStreamer/MPP） |
...
| `overlay/board/rockchip/common/rootfs-overlay/` | Generic OS overlay (systemd, helpers, Weston; **不含** App bundle — 见 `app/*/build/bundle/release`) |
| `overlay/board/rockchip/$CHIP/rootfs-overlay/` | SoC-thin extras (current line: `rk3566_rk3568`; may be empty) |
| `overlay/buildroot/` | Defconfig fragments, package pins |
| `overlay/kernel/` | DTS / kernel config — **board trees:** `overlay/kernel/rockchip/` (`ynh960.dts`, `customer_board_*.dtsi`, …) |
