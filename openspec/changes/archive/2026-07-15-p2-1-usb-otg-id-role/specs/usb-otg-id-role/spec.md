...
Because Micro-USB **ID may be unwired or OTG adapters leave ID floating**, the system SHALL NOT require IDDIG auto-role for Demo/product control. The image SHALL provide `/usr/lib/lws-hmi/usb-otg-mode.sh` with commands `debug` / `host` / `status` / `apply` that select Micro-USB **peripheral (Debug over USB / plug-ssh)** vs **host (keyboard)**. Preference SHALL be stored at `/var/lib/lws-hmi/usb-debug` (`1` = debug, `0` = host). When the preference file is missing, Debug over USB SHALL default to **on**.

#### Scenario: Default Debug over USB on

- **WHEN** `/var/lib/lws-hmi/usb-debug` is absent and `usb-otg-mode.sh apply` runs
...
- **THEN** preference is `0`, `otg_mode` is `host`, and `lws-hmi-usb-plug-ssh.service` is stopped so a keyboard on an OTG adapter can enumerate

#### Scenario: Preference survives reboot via userdata bind

- **WHEN** Debug over USB was set off and the board reboots with `/var/lib/lws-hmi` bound to userdata
