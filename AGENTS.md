...
| `scripts/upgrade-remote.sh` (SSH host-HTTP + device pull / RockUSB di), Makefile `upgrade` only (board already has P2.4 overlay + A/B GPT + OTA HMI) | `make upgrade` (or `OEM_ONLY=1 make upgrade`); no firmware rebuild unless image inputs are stale; App must understand `download <url>` (`make build-app` / `push-app` if watcher is old) |
