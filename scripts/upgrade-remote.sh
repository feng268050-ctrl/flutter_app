...
directly into partitions with progress, arms try-boot, and returns as soon as
reboot is requested (does not wait for the board to come back).
...
# Fire-and-forget: arm-reboot sets apply.status=ok then reboots. Do not poll SSH
# or wait for disconnect — ControlMaster often hangs across USB-gadget reboot,
# and the host only needs confirmation that reboot was requested.
set +e
