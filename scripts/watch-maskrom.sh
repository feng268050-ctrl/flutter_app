#!/usr/bin/env bash
# Poll RockUSB via upgrade_tool while you try MaskROM (Recovery LED may stay off — that's OK).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
case "$(uname -s)" in
Darwin) UPGRADE_DIR="$ROOT/tools/upgrade_tool/macos"; UPGRADE="$UPGRADE_DIR/upgrade_tool" ;;
Linux) UPGRADE_DIR="$ROOT/tools/upgrade_tool/linux"; UPGRADE="$UPGRADE_DIR/upgrade_tool" ;;
MINGW* | MSYS* | CYGWIN*)
  UPGRADE_DIR="$ROOT/tools/upgrade_tool/windows"
  UPGRADE="$UPGRADE_DIR/upgrade_tool.exe"
  ;;
*)
  echo "watch-maskrom: unsupported host OS $(uname -s)" >&2
  exit 1
  ;;
esac
SEC="${WATCH_SEC:-300}"

[[ -x "$UPGRADE" ]] || chmod +x "$UPGRADE" 2>/dev/null || true
...
重要：
  ...
  • MaskROM = 芯片 ROM，与 eMMC 内容无关；只要 USB OTG + 按键/供电正确，主机应能看到 0x2207 设备
...
ynh960 建议顺序（每种多试 2–3 次）：
  ...
  D) 换线 / 换 Hub / 换主机口；禁用 USB 3 扩展坞，用 USB2 Hub
...
for ((t = 0; t < SEC; t += 2)); do
  ts="$(date '+%H:%M:%S')"
  rock=""
  rock="$(cd "$UPGRADE_DIR" && "$UPGRADE" ld 2>&1 | grep -v '^Using ' || true)"
  ioreg=""
...
3. Windows：确认已装 Rockchip USB 驱动（DriverAssistant）；Linux：udev 规则允许访问 vendor 2207。
