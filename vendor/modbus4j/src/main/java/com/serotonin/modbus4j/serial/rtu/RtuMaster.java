...
public class RtuMaster extends SerialMaster {
    ...
    // 上一次发送失败的时间
    private volatile long lastSendErrorTime = -1;
    ...
    public ModbusResponse sendImpl(ModbusRequest request) throws ModbusTransportException {
        ...
        try {
            ...
            if (lastSendErrorTime > 0 &&
                    (time - lastSendErrorTime) < ModbusConfig.getSendErrorTryConnectionTimeInterval()) {
                LOG.debug("发送失败，当前时间间隔内，不进行重新连接");
                return null;
            }
            lastSendErrorTime = time;
            try {
