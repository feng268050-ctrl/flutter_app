...
- Add host tooling: extend **`make devices`** to a **single merged table** (RockUSB + USB-SSH rows with **`MODE`** column: `Loader` / `Maskrom` / `USB-SSH`; columns `SERIAL`, `LocationID`, `IFACE`, `ADDR`), and **`make push-hmi`** (`SERIAL=` when multiple boards) to `scp` artifacts and `ssh systemctl restart hmi.service`.
- Extend **`make bootloader`** (existing `scripts/flash-usb.sh`) so a board running **Linux** reboots into **RockUSB Loader** via **`ssh … /usr/lib/lws-hmi/reboot-rockusb-loader`** over the USB ECM link (same `SERIAL=` / `BindInterface` as `push-hmi`); retain **adb reboot loader** as fallback when adb is available.
...
- `host-push-hmi`: Host `make devices` / `make push-hmi`, USB serial → host interface resolution (`BindInterface`), `SERIAL=` multi-device selection aligned with `scripts/flash-usb.sh`, integration with `make build-flutter-app` outputs; extend **`make bootloader`** for Linux via USB-SSH + `reboot-rockusb-loader`.
...
- **Host**: `scripts/push-hmi.sh`, `scripts/usb-ssh-devices.sh` (names TBD), Makefile targets; extend `scripts/flash-usb.sh` **`bootloader`** for Linux USB-SSH path; README / AGENTS.md rebuild table row for overlay-only vs app-only.
