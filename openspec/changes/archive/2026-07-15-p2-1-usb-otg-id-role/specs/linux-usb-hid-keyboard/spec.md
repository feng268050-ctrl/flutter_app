...
The system SHALL support a wired **USB HID keyboard** attached to the on-board **Micro-USB** jack when Debug over USB is **off** (OTG host role), such that the kernel enumerates the device and exposes an input event node under `/dev/input/` usable by flutter-pi, in addition to the existing **1 mm pin-header USB host expansion** path.

#### Scenario: Keyboard on OTG after host mode

- **WHEN** Debug over USB is off and an operator connects a USB HID keyboard through an OTG adapter on Micro-USB
...
With a focused text input in the HMI Flutter app running under flutter-pi, printable keys and common editing keys from the USB HID keyboard SHALL be delivered through the platform input path (evdev/libinput → flutter-pi → Flutter) without requiring a Dart soft-IME, whether the keyboard is attached via the **1 mm host expansion** or via **Micro-USB host** (Debug over USB off).
...
Keyboard host bring-up on the **1 mm expansion** SHALL NOT require unloading plug-ssh `g_ether` or toggling Debug over USB. Soft on-screen keyboard (FrostIME) is out of scope. Keyboard on Micro-USB SHALL use Debug over USB **off** (host) per `usb-otg-id-role` and MUST NOT require plug-ssh to stay loaded in host mode.

#### Scenario: Concurrent Micro-USB plug-ssh and expansion host keyboard

- **WHEN** Debug over USB is on (peripheral plug-ssh) and a USB HID keyboard is attached via the 1 mm host expansion
- **THEN** keyboard enumeration and typing smoke MUST still be possible without switching Debug over USB for the expansion keyboard
