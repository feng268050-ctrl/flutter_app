...
- **Rename host device-selection env** from `SERIAL=` / `LWS_HMI_SERIAL=` to **`SN=` / `LWS_HMI_SN=`** across Make targets (`push-app`, `upgrade`, `shell`, `flash`, `reboot*`, `debug-app`, `set-prop` selection, …). `SN=` matches table **SN** or **ChipID**. Add **`CHIPID=`** for ChipID-only selection (needed when `make set-prop SN=…` would collide with device selection). Keep deprecated `SERIAL=` as an alias. Update Makefile `help`, `.env.example`, README, and AGENTS.md.
...
- Add host Make targets mirroring lws-ui **`make set-prop` / `make del-prop`**, targeting `/var/lib/hmi/product.ini` over SSH (USB-SSH or registered `IP`/`SN`/`CHIPID`), not adb/`model.properties`.
  ...
  - When `SN=` is a **product** assignment on `set-prop` / `del-prop`, it MUST NOT also act as device selection for that run (use `CHIPID=` / `IP=` / deprecated `SERIAL=` on multi-board).
...
- Docs: note Linux path `/var/lib/hmi/product.ini` as the successor to the P5.1 `model.properties` candidate for these keys (no Android `/system/etc/model.properties` work in this change); document `SN=` / `CHIPID=` / `set-prop` / `del-prop` in Makefile `help`, README Make commands, and AGENTS.md.
...
- **Host tooling:** `scripts/set-product-prop.sh` / `scripts/del-product-prop.sh` + Makefile `set-prop` / `del-prop`; rename device-selection env `SERIAL=` → `SN=` (+ `CHIPID=`); update `help`, README, AGENTS.md (host-only; no firmware rebuild).
