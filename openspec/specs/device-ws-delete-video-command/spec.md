...
Define device-side handling for deleting process videos over WebSocket: inbound `command.delete_video` with business `video_id`, outbound `command.delete_video_ack` (same payload shape as `command.upload_video_ack`), and shared delete semantics with local HTTP `DELETE /v1/videos/:video_id`.
