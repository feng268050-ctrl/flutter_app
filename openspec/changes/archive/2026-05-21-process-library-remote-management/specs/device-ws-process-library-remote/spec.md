...
- **WHEN** the database contains `QUICK_MODE_DATA` or `VIDEO_PROCESS_DATA` rows for the same `processType`
...
Inbound frames with `type` **`command.process_parameters_delete`** SHALL include **`id`** in `payload` (string or number). The device SHALL delete the row only when **`dataType`** is **`ENGINEER_MODE_CUSTOM_DATA`**.

#### Scenario: Delete custom row

- **WHEN** `payload.id` references a custom engineer row
- **THEN** the device MUST delete the row and MUST send `command.process_parameters_delete_ack` with `success` true

#### Scenario: Reject delete default row

- **WHEN** `payload.id` references `ENGINEER_MODE_DEFAULT_DATA`
- **THEN** the device MUST NOT delete the row and MUST send ack with `success` false and a message indicating default presets cannot be deleted remotely

### Requirement: Inbound process parameters set default

Inbound frames with `type` **`command.process_parameters_set_default`** SHALL include **`id`** in `payload` only (string or number; no other business fields required). The device SHALL load the row by **`id`**, reject when missing or not engineer-mode (`dataType` 1 or 2), then SHALL set the active engineer preset for that row’s **`processType`** (memory cache key `ENGINEER_DATA_CACHE_KEY + processType`, same behavior as UI `switchProcessParametersData`).
