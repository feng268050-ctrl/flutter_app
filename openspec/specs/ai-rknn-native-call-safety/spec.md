...
Define guardrails for RKNN runtime calls so invalid lifecycle, concurrency, or input-boundary states are rejected before they can produce native fatal signals.
...
Before calling `rknn_inputs_set`, `rknn_run`, or `rknn_outputs_get`, the system SHALL validate required fields including buffer pointer presence, alignment constraints, size/index/fmt consistency, and session state. Invalid calls SHALL fail fast before entering RKNN runtime APIs.
