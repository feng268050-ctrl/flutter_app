...
2. **RockUSB OTA-images (when Loader/Maskrom is selected):** flash the OTA-equivalent loose images via partition download as specified in the RockUSB requirements of this capability — **not** product/unified OTA zip+sign apply, and **not** `make flash` factory packaging.
