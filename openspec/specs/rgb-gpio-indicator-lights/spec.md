...
Operator-facing chassis RGB GPIO policy for the LWS HMI App (parity with lws-ui `RgbLedDecision` / `GpioLedHandler`): red = laser emit/standby/offline, yellow = yellow-severity coded alarms, green = ready-to-emit. Pin plumbing stays in `linux-gpio-rgb-led`; this capability covers when each Steady/Blink/Off applies, refresh sources, boot/settings Off reset, and LED Settings manual override.
