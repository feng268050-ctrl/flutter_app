...
When `BuildConfig.RELEASE_CHANNEL` is `false`, before calling `nativeOpencvZeroPointDetectFromI420` for a zero-point sample, the system SHALL attempt to read and parse the mock file. If parsing succeeds, the system SHALL use that sample and SHALL NOT call native detect for that sample.
...
- **THEN** sample generation SHALL fall back to native `nativeOpencvZeroPointDetectFromI420` unchanged
