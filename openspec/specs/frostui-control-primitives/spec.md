# frostui-control-primitives Specification

## Purpose
TBD - created by archiving change frostui-control-primitives. Update Purpose after archive.
## Requirements
...
#### Scenario: Process video seek uses direct drag

- **WHEN** `FrostFlankedSliderView` is used for playback position
- **THEN** its embedded `FrostSliderView` MUST have `longPressDragEnabled=false`
- **AND** tap/drag on the track MUST update progress immediately without long-press arming

#### Scenario: Track tap does not jump value on advanced settings sliders

- **WHEN** the user taps the linear slider track away from the thumb on a `FrostSliderView` with long-press enabled
