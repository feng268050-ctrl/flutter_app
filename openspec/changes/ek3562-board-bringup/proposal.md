...
ek3562 (RK3562 EVB2 DDR4 V10) already has Linux board DTS in `overlay/kernel/rockchip/` and OEM placeholders, but is not yet a first-class FIT conf, self-built bootloader pair, or flashable `FACTORY_SKU`. After ynh960 proves the self-built SPL **`rk356x_spl_loader_v*.bin`** (rkbin OUTPUT) + Linux-first U-Boot path (and rollback), we can apply the same rules to ek3562 without learning unbrick on a less familiar board first.

## What Changes

- Formalize **ek3562** as a product `board_id`: FIT inventory line, OEM `compat.fit_dt: ek3562`, factory SKU `ek3562-dev` with real `prebuilt/bootloader/vendor-ek3562/{rk3562_spl_loader_v*.bin,uboot.img}`.
- Self-build the early loader via rkbin `boot_merger` + `RK3562MINIALL*.ini` (DDR-matched to EVB2), shipping the **OUTPUT basename** **`rk3562_spl_loader_v*.bin`** as-is (not inventing `loader.bin` / `bootloader.bin`).
...
### New Capabilities
- `ek3562-board`: End-to-end ek3562 board identity — FIT conf, OEM fit_dt, factory SKU bootloader pair (`rk3562_spl_loader_v*.bin` + Linux-first `uboot.img`), and bring-up acceptance criteria.
...
- `buildroot-lws-hmi-image`: `FACTORY_SKU=ek3562-dev` packaging MUST consume `prebuilt/bootloader/vendor-ek3562/` with **`rk3562_spl_loader_*.bin`** naming consistent with the ynh960 rkbin-OUTPUT policy (`rk356x_spl_loader_*.bin`).
