...
- Add/extend verb-noun scripts under `/usr/libexec/hmi/` that write the canonical preference files used by `restore-settings.sh` / `hmi-launch.sh`.
...
- `linux-settings-persist`: Clarify that simple HW prefs are written by shell apply commands; Flutter controllers call helpers rather than writing those `/var/lib/hmi/*` files directly.

## Impact

- Overlay: `change-backlight.sh` (complete persist if not already), new `change-volume.sh` / `change-orientation.sh` / `apply-mouse-settings.sh` (names TBD in design), `restore-settings.sh` callers, `post-build.sh` PATH links, `verify-rootfs-overlay.sh` / `env-verify.sh`.
