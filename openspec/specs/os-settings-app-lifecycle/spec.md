# os-settings-app-lifecycle Specification

## Purpose
Board lifecycle for OS Settings seat: static `os-settings.service`, switch helpers / CLI, bidirectional Conflicts with `hmi.service`, HMI entry and failure Toast.
...
The product HMI SHALL expose OS Settings entry via Device Information → **five consecutive taps on Device SN**, invoking `switch-to-os-settings`. On missing `/opt/os_settings` bundle or `systemctl start os-settings` failure, the HMI SHALL show a Toast (or equivalent) and MUST remain on the HMI seat. Hidden multi-tap on Kernel Version MUST NOT be the primary entry path. A visible Common Settings “OS Settings” row MUST NOT be required.
