# stain-audit-auto-upload Specification

## Purpose
TBD - created by archiving change stain-audit-auto-upload. Update Purpose after archive.
## Requirements
...
### Requirement: DETECT_FAILED enqueue SHALL use single-image Work and bound disk growth

When resolving `input_frame.jpg`, the App SHALL accept absolute paths in summary `files[]`, paths relative to `{filesDir}/lens_guard`, and a fallback `lastOutputDir/frame_<id>/input_frame.jpg` from the last successful StreamDetect configure.

DETECT_FAILED upload SHALL schedule the same WorkManager worker as Pictures batch (`AiUploadSingleImageWorker` → Worker `ai-report`), after copying the frame into an app-owned inbox (`files/ai_audit_inbox/`). The Live path SHALL NOT append to `files/ai_upload/.../queue/pending.json` for this V1 audit flow.

After a successful inbox stage + Work enqueue, the App SHALL delete the native per-frame directory that held the source `input_frame.jpg`. After ai-report success, the worker SHALL delete the inbox image when eligible.

The App MAY throttle Live DETECT_FAILED enqueue to at most one successful enqueue per 15 seconds; throttled frames SHALL discard their native `input_frame.jpg` / `frame_*` directory without enqueue. On LiveInfer grace end or StreamDetect session stop, the App SHALL best-effort purge remaining `frame_*` directories under the last session output directory.

#### Scenario: Relative file path under lens_guard resolves

- **WHEN** summary `files[]` contains a path relative to the daemon workdir (`lens_guard`)
- **AND** the file exists under `{filesDir}/lens_guard/...`
- **THEN** enqueue SHALL locate `input_frame.jpg` and proceed

#### Scenario: Enqueue success drops native frame dir

- **WHEN** DETECT_FAILED enqueue stages the image under `files/ai_audit_inbox/` and schedules `AiUploadSingleImageWorker`
- **THEN** the App SHALL delete the corresponding native `frame_<id>/` directory
