...
Board + host contracts for HMI-driven screenshot and screen recording via shared `packages/cyber_capture` (Flutter-paced frames, eLinux present-hook + C encode library + GStreamer/MPP/RGA, host Make trigger/pull), reusable across product `*_hmi` Apps.
