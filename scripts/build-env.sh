...
setup_build_env() {
  ...
  local nproc_wrapper_dir="${NPROC_BIN:-/tmp/lws-hmi-nproc-$$}"
...
EOF
  ...
  if [[ "${NO_MAKEFLAGS:-}" == "1" || "${DOCKER:-}" == "1" ]]; then
    unset MAKEFLAGS
  else
  ...
  if [[ "${DOCKER:-}" == "1" || -f /.dockerenv ]]; then
    local curl_home="${DOCKER_ROOT:-/work/lws-hmi}/.cache/curl-home"
    mkdir -p "$curl_home"
