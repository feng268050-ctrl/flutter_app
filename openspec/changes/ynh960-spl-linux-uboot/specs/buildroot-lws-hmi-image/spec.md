...
#### Scenario: Packaging prefers loader.bin

- **WHEN** `prebuilt/bootloader/<uboot_id>/loader.bin` exists for the resolved SKU
- **THEN** `make build-img` SHALL use that file as the early loader input (MUST NOT require a differently named opaque blob as the only accepted name)
