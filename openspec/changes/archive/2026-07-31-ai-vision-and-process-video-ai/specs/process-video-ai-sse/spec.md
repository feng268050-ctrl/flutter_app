...
### Requirement: Sample grid and offline infer

Process-video sessions SHALL sample at 500 ms intervals (first sample at 500 ms; 0 ms never sampled), extract a JPEG via App-bundled ffmpeg, and invoke daemon `offline_infer_opencv_stain_jpg`. Results SHALL append to an in-memory timeline and fan out on the session SSE hub.

#### Scenario: Sample at interval

- **WHEN** the playback clock reaches a new sample bucket
- **THEN** the session MUST extract one frame and request offline JPG infer
