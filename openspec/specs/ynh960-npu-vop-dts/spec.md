# ynh960-npu-vop-dts Specification

## Purpose

ynh960 EVB-derived device tree wiring for NPU bus OPP (`pvtm-supply`), VOP OPP (`vop-supply`), and RKNPU named IRQ (`npu_irq`), applied from git-owned `overlay/kernel/` via `apply-overlay`.
