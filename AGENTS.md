...
| `properties.ini` host tooling (`scripts/set-product-prop.sh`, `scripts/del-product-prop.sh`, Makefile `set-prop` / `del-prop`) | none (host SSH mutate `/var/lib/hal/properties.ini` tunables only; brand/model/sn refused → `make write-identity`); exercise `make set-prop` / `make del-prop` (multi-board: `SN=` / `CHIP_ID=` / `IP=`) |
