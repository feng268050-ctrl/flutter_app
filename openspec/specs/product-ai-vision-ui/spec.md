...
The AI Vision tab SHALL draw detection boxes + right-top HUD aligned with lws-ui `bindDetectionOverlayHud` / `updateOfflineInferenceOverlay` / `updateLiveInferenceOverlay`:

- Stain-detect HUD status SHALL be `AI: STAIN_DETECT` without a "Latest result" detail line.
- Selected-video idle (cover, Detect or Replay/Re-detect resident) SHALL show `AI: IDLE` only and MUST clear boxes.
- Selecting a process video without a persisted timeline SHALL auto-start Detect (playback + inference).
- Live StreamDetect SHALL show boxes/HUD only when a fresh target exists; otherwise clear the overlay.
- During Detect/Replay playback, overlay frames SHALL be sampled from the timeline at the current playback position on a ~250ms tick (`findFrameAt`), not only from the latest SSE append.
- Overlay updates MUST NOT rebuild the video player on every tick (avoid playback stutter).
- Boxes SHALL map through fit-center content geometry (pixel → normalized → content rect) with `#00E676` stroke and optional `contamination` labels.

Choosing a process video SHALL show metadata (process type, material, create time). Detect SHALL start/reuse `ProcessVideoAiSession` with UI holder; Replay SHALL load timeline frames; Re-detect SHALL force-restart inference.
...
- **THEN** inference overlay updates MUST appear during the session synced to playback position

#### Scenario: Auto Detect on first select

- **WHEN** the user selects a process video with no persisted timeline
- **THEN** Detect MUST start automatically without requiring a second Detect tap

#### Scenario: Selected idle HUD

- **WHEN** a process video is selected and Detect/Replay is not playing
- **THEN** the HUD MUST show `AI: IDLE`
- **AND** detection boxes MUST be cleared

#### Scenario: Stain HUD without result detail

- **WHEN** a stain-detect sample with a target is shown
- **THEN** the HUD MUST show `AI: STAIN_DETECT`
- **AND** MUST NOT show `Latest result: …` / waiting detail for that path
