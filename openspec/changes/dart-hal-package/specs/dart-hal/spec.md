...
Physical USB/BT HID keyboard layout SHALL be applied through flutter-pi / libxkbcommon (XKB), driven by a preference file under `/var/lib/hmi/keyboard.conf` (layout / variant / options / model). `hal/input/keyboard` SHALL expose get/set/list layout APIs that write this pref. flutter-pi SHALL reload the keymap on pref change without restarting `hmi.service` and without `SIGHUP`. Soft-keyboard layouts remain CyberIME and MUST NOT be implemented as Dart remapping of HID scancodes. `/etc/default/keyboard` MAY remain the image default; runtime preference SHALL take precedence while the HMI is running.

#### Scenario: US to Russian
- **WHEN** the App calls setLayout for `ru` (or equivalent) with xkeyboard-config present
- **THEN** subsequent physical key events SHALL produce Russian layout characters after flutter-pi reloads XKB from `keyboard.conf`

#### Scenario: No HMI restart for layout
- **WHEN** layout preference changes mid-session
- **THEN** the new layout SHALL apply without `systemctl restart hmi` and without sending `SIGHUP` to flutter-pi
