...
Deployment SHALL stage files under **`/var/lib/hmi/push-app-staging/`**, replace `/opt/hmi` app artifacts while the current HMI remains running, sync storage, then restart `hmi.service`. If the restart does not leave eLinux HMI active, the helper SHALL reset the systemd failure state and retry activation a bounded number of times.
