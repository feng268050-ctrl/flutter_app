...
if ! usb_ssh_session_run_ssh "$ROOT" "$IFACE" "test -x /usr/libexec/hmi/debug-app-run.sh"; then
	echo "ERROR: /usr/libexec/hmi/debug-app-run.sh not found on board" >&2
	exit 1
fi

usb_ssh_session_run_ssh "$ROOT" "$IFACE" /usr/libexec/hmi/debug-app-run.sh
