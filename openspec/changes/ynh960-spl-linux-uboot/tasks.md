...
- [ ] 1.1 Create dated backup of current `prebuilt/sdk-loader/MiniLoaderAll.bin` and `prebuilt/sdk-uboot/uboot.img` (resolve symlinks) under `prebuilt/bootloader/rockchip-ynh960/backup/<stamp>/` with README (sizes, `ddr-v*`, `bl31-v*`, `bl32-v*`)
- [ ] 1.2 Define install layout: `loader.bin` + `uboot.img` under `prebuilt/bootloader/rockchip-ynh960/`; document optional `MiniLoaderAll.bin` → `loader.bin` symlink for tool compat
...
- [ ] 2.2 Run `boot_merger`; install output as `loader.bin`; record ini + output name in package README
- [ ] 2.3 Update `scripts/build-img.sh` (and flash/Maskrom helpers) to prefer `loader.bin`, keep transitional fallback
...
- [ ] 3.4 Update docs (`docs/uboot-rkbin.md`, make-commands, package README); remove stale “never build uboot” / MiniLoader-only wording

## 4. Factory package and lab validation

- [ ] 4.1 `FACTORY_SKU=ynh960-p800 make build-img` with new pair; confirm factory packages `loader.bin`
