...
- **Generic OS vs product vs hardware (do not conflate):**
  ...
  - **New SoC on same product line** — board vendor drops **DTS, display params, U-Boot/MiniLoader** into repo (`overlay/kernel/`, `board/`, `prebuilt/bootloader/`, `oem/`); **no second `linux-sdk`**. Merge SoC drivers into the **same universal Image** Kconfig; add `board_id` to FIT inventory. Rootfs userspace recipe stays shared; products stay **`APP=`** unless userspace ABI diverges. Detail: [`docs/make-commands.md`](docs/make-commands.md) → **新板 / 新 SoC 接入**.
...
| `overlay/kernel/**/ynh960-selinux.config` + `chips/lws_hmi_selinux.config` (SELinux permissive + auditd) | `make apply-overlay`, `bash scripts/br-make-packages.sh selinux libselinux libsepol refpolicy policycoreutils libsemanage audit systemd`, **`FORCE_KERNEL_IMAGE=1 make build-kernel`**, `make build-rootfs`, `make upgrade` — **not** `make build-uboot`; force-rebuild AIC `.ko` if `CONFIG_SECURITY` just flipped (see [`docs/selinux.md`](docs/selinux.md) Wi‑Fi note) |
