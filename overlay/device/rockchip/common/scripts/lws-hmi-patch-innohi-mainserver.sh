...
target="$1"
marker='lws-hmi: systemd + flutter-pi skips Innohi MainServer/S99-init rebuild'
...
pat = re.compile(
    ...
    rf"\tsudo cp -rf \$\{{CROOT\}}/innohi_board/rootfs/usr/bin/MainServer  usr/bin/\n"
    rf"\tsudo cp -rf \$\{{CROOT\}}/innohi_board/rootfs/usr/bin/ParamUpdate  usr/bin/\n"
    ...
    rf"\tcd -;\n"
    rf"\tcp -rf \$\{{CROOT\}}/innohi_board/rootfs/usr/bin/\*  "
...
guarded = """\t\t
# lws-hmi: systemd + flutter-pi skips Innohi MainServer/S99-init rebuild.
...
\tcd -;
\tcp -rf ${CROOT}/innohi_board/rootfs/usr/bin/*  ${CROOT}/buildroot/output/${RK_BUILDROOT_CFG}/target/usr/bin/
\t
