...
The repository SHALL provide **`make reboot`** that reboots a connected board **without** entering RockUSB Loader: Linux HMI via USB-SSH **sysrq reboot**; Android via **`adb reboot`**. **`make reboot-loader`** is the Loader entry path.
...
- **WHEN** no USB-SSH target is available and adb reports a connected Android device
