...
Product Monitor UI paths for Machine Status Device Health temperatures and boolean alarms SHALL reference stable attribute ids from the product App modbus config asset (e.g. `telemetry.gun_motor_temp`, `alarm.gun_motor_over_temp`, `alarm.gun_comm`). Those paths MUST NOT hard-code numeric Modbus addresses or bit indices in Dart UI/application code after this capability lands. Adding a missing Monitor field SHALL be done by extending the config catalog (and optionally meta), not by embedding addresses in the widget tree.
