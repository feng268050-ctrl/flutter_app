...
if [ -f /run/wpa-wlan.iface ]; then
	IFACE="$(tr -d '[:space:]' </run/wpa-wlan.iface)"
	IFACE="${IFACE:-wlan0}"
fi
...
if command -v systemctl >/dev/null 2>&1; then
	...
	systemctl stop wlan-wpa.service 2>/dev/null || true
	systemctl reset-failed wlan-dhcp.service 2>/dev/null || true
	systemctl reset-failed wlan-wpa.service 2>/dev/null || true
fi
...
rm -f /var/lib/wpa_supplicant/wifi-wanted
