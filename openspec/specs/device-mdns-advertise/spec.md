# device-mdns-advertise Specification

## Purpose

Advertise `_lws-device._tcp` on the LAN so LaserCyber mobile can discover the device and reach the working HTTP API on port `5580`. Advertisement is gated by local HTTP health and usable LAN addressing, not by cloud reachability.
