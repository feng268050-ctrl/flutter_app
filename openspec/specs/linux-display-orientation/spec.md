...
Reusable display-orientation API (portrait / landscape) persisted for the eLinux HMI `-o` mapping on HMI restart.
## Requirements
...
### Requirement: Linux maps modes to the eLinux client launch orientation
...
- eLinux HMI: `landscape` → `-o landscape_left`, `portrait` → `-o portrait_up`
...
- **WHEN** the client sets portrait (via HAL or `change-orientation`) and the HMI process is restarted via the normal launch path on the eLinux HMI
- **THEN** HMI starts with `-o portrait_up` and the UI is in portrait

#### Scenario: Landscape restores production default

- **WHEN** the client sets landscape and the HMI process is restarted on the eLinux HMI
- **THEN** HMI starts with `-o landscape_left`
...
Setting orientation on Linux MAY restart the HMI process to apply eLinux HMI `-o` or Weston `transform`. The operation SHALL NOT brick boot: if preference write fails, the previous orientation remains in effect.
