...
Define how the LWS Android app persists **process video** rows (`t_params_process_video`), uploads **video metadata** (multipart, including a JPEG cover frame) to the LaserCyber Worker **`POST /v1/devices/:sn/videos/metadata`** on the pinned API base, drains pending rows after network/API pin is available (WorkManager), and keeps metadata work off the UI thread.
## Requirements
