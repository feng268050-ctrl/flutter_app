...
USB_SSH_SYSRQ_REBOOT_CMD='sh -c "(sleep 1; sync; if [ -w /proc/sysrq-trigger ]; then echo 1 >/proc/sys/kernel/sysrq 2>/dev/null; echo b >/proc/sysrq-trigger; elif [ -x /usr/libexec/power/shutdown.sh ]; then /usr/libexec/power/shutdown.sh reboot; fi) >/dev/console 2>&1 & exit 0"'
