# device-cloud-access-token-api Specification

## Purpose

Attach and refresh device Bearer (`access_token`) on existing v1 Worker HTTP and `/ws/device` after Ed25519 token mint; bootstrap activate/token remain unauthenticated. Aligns HMI clients with api-server `device-access-token-auth`.
