# Hmi App Cloud Ota Specification

## Purpose

Independent HMI app OTA channel: HMI Version identity, Settings HMI Upgrade UI, CDN `lws-hmi/app/release.json` check, and signed `tar.gz` install to `/opt/hmi` with `hmi.service` restart (no A/B partition writes).
