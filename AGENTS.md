...
|--------------|----------|
| `app/lws_hmi/**`, `scripts/build-app.sh`, `scripts/hmi-bundle-common.sh`, `scripts/upgrade-app.sh`, `scripts/pack-app.sh`, `scripts/push-app.sh`, `scripts/app-select.sh` | `make build-app`, then `make push-app` (Debug unsigned) or `make upgrade-app` (signed). Other apps: `APP=<id> make build-app` / `push-app` / `upgrade-app` |
...
| `overlay/kernel/**/ynh960-selinux.config` + `chips/lws_hmi_selinux.config` (SELinux permissive) | `FORCE_PLATFORM_OVERLAY=1 make apply-overlay`, `bash scripts/br-make-packages.sh selinux libselinux libsepol refpolicy policycoreutils libsemanage systemd`, force-rebuild AIC `.ko` if `CONFIG_SECURITY` just flipped (see [`docs/selinux.md`](docs/selinux.md) Wi‑Fi note), `make build-kernel`, `make build-rootfs`, `make upgrade` — **not** `make build-uboot` |
