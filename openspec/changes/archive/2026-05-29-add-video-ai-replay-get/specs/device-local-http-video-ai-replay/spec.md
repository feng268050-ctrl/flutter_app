...
- `frames` (array, required) where each element includes:
  - `streamTimeMs` (number, required): media timeline position in milliseconds
  - `timestampMs` (number, required): completion time associated with the inference sample
  - unified inference fields compatible with SSE inference payload semantics (e.g. `success`, `code`, `level`, `status`, `message`, `imageWidth`, `imageHeight`, `boxes`, `source`)

#### Scenario: Client replays by streamTimeMs

- **WHEN** the device returns replay JSON for video `V`
- **THEN** the `frames` array MUST allow a client to select the best entry for a given playback time using `streamTimeMs`
