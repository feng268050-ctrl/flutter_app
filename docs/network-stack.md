...
| DNS | **systemd-resolved** | networkd feeds DNS to resolved; `/etc/resolv.conf` → `../run/systemd/resolve/resolv.conf`. LLMNR/mDNS off; **DNSSEC=no** and **DNSOverTLS=no** until product NTP (wrong RTC → `signature-expired` breaks all names). Do **not** hand-write resolv from helpers. |
