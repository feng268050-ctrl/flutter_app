# host-register-device Specification

## Purpose

Host `make register-device`: select board via `SN=`/`IP=`, read Vendor Storage identity over SSH, register with api-server admin devices API.
