...
- Enable **Plan A systemd boot chain**: `hmi.service` + Innohi `mainserver.service` + `cpu-performance.service` + `pwrkey-poweroff.service`; defer Wi‑Fi/BT/network at boot; keep mediamtx / sshd / bluetoothd **disabled**.
...
- **Overlay**: Rootfs overlay for `/opt/hmi` Hello World artifacts; U-Boot logo under `board/logo/`; systemd units and helper scripts under `usr/libexec/{wpa,network,bluetooth,hmi}/`.
...
- **Existing overlays reused**: `lws_hmi_{base,systemd,network}.config`, `hmi.service`, `06-systemd-appliance.sh`, `08-systemd-appliance-finalize.sh`, LCD/MIPI display overlay.
