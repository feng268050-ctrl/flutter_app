...
void main() {
  testWidgets('shows P2 device information labels', (WidgetTester tester) async {
    ...
    expect(_textAnywhere('SoC Temperature'), findsOneWidget);
    expect(_textAnywhere('GPU Temperature'), findsOneWidget);
    expect(_textAnywhere('42.5 °C'), findsOneWidget);
    expect(_textAnywhere('39.0 °C'), findsOneWidget);
    expect(_textAnywhere('Motor Temperature'), findsOneWidget);
    expect(_textAnywhere('Motor Driver Temperature'), findsOneWidget);
    expect(_textAnywhere('Protective Mirror Temperature'), findsOneWidget);
    expect(_textAnywhere('Collimator Temperature'), findsOneWidget);

    await tester.scrollUntilVisible(
      _textAnywhere('RGB LED'),
      300,
      scrollable: _mainScrollable(),
    );
    expect(_textAnywhere('RGB LED'), findsOneWidget);

    await tester.scrollUntilVisible(
      _textAnywhere('Speaker'),
      400,
      scrollable: _mainScrollable(),
    );
    expect(_textAnywhere('Play'), findsOneWidget);
    expect(find.textContaining('Volume:', skipOffstage: false), findsOneWidget);

    await tester.scrollUntilVisible(
      _textAnywhere('Backlight'),
      400,
      scrollable: _mainScrollable(),
    );
    expect(_textAnywhere('Backlight'), findsOneWidget);
    expect(
      find.textContaining('Brightness:', skipOffstage: false),
      findsOneWidget,
    );
  });
