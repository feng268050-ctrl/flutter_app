...
Settings cards and action buttons SHALL use reusable FrostedGlass components. Card-like containers MUST use `FrostedGlassCard`, and button-like actions MUST use `FrostedGlassButton` instead of image-resource-backed background assets.

#### Scenario: Settings card renders
- **WHEN** a Settings page displays a grouped card or row container
- **THEN** the container uses `FrostedGlassCard` for its visual chrome
...
- **THEN** the action uses `FrostedGlassButton`
...
- **THEN** the Zero Offset `Auto` action uses `FrostedGlassButton` with `borderGradientCenter="left-right"`
...
- **THEN** Network contains Wireless Network
...
Advanced Settings SHALL display device parameters in titled groups: Offset & Correction, Power Thresholds, and Temperature Thresholds.
