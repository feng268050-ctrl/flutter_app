...
- [ ] 1.3 Confirm sibling api-server change **`hmi-ota-static-upload`** is implemented/deployed (or use documented interim bridge only if agreed)
...
- [ ] 2.2 Wire Makefile `publish` (depends on `ota-package`) and `publish-only`; rename/copy zip to server basename if needed; require `PUBLISH_API_TOKEN` / base URL from env + `.env`
...
- [ ] 3.3 Cross-link `unified-ota-cyber-ota` / ota-package: upgrade and publish share the same zip; point Worker gaps to api-server **`hmi-ota-static-upload`**
