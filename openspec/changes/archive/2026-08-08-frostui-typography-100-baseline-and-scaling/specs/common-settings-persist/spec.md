...
Common Settings preferences that are neither HAL/platform-backed nor Misc-section toggles SHALL be persisted in a single JSON file at `/var/lib/hmi/common-settings.json` (or `${OsPaths.varHmi}/common-settings.json`). The App SHALL NOT introduce additional per-preference files under `/var/lib/hmi/` for Language, Unit, Country, Text Size, or future peers of this class. Keys for at least Language, Unit, Country, and Text Size SHALL live in this file. Language wire values SHALL be BCP-47 tags `en-US`, `zh-CN`, or `zh-TW`. Country wire values SHALL be ISO 3166-1 alpha-2 uppercase codes from the product Country catalog (full ISO list plus `XK`). Text Size wire values SHALL be `small`, `medium`, or `large` (case-sensitive lowercase). Missing file or missing keys SHALL apply documented per-key defaults (`language` = `en-US`, `unit` = `Metric`, `country` = `US`, `textSize` = `medium`). On read, legacy Language values `EN` and `ZH` SHALL be accepted and normalized to `en-US` and `zh-CN` respectively. Unsupported Country codes MUST normalize to `US`. Unsupported or missing Text Size values MUST normalize to `medium`. Corrupt JSON MUST NOT crash the App (soft-fail to defaults including Country `US` and Text Size `medium`). Misc toggles MUST remain in `misc-settings.json`. HAL-backed Common Settings (brightness, AutoSleep, volume, ButtonFeedback sound-effect, network, datetime, mouse, keyboard, USB OTG, etc.) MUST NOT be relocated into `common-settings.json`.

#### Scenario: Defaults when file absent

- **WHEN** `/var/lib/hmi/common-settings.json` is absent
- **THEN** Language is `en-US`, Unit is `Metric`, Country is `US`, and Text Size is `medium` until the operator changes a value
...
#### Scenario: Text Size change persists

- **WHEN** the operator selects Text Size `large`
- **THEN** `/var/lib/hmi/common-settings.json` is updated so `textSize` is `large`
- **AND** a subsequent process start restores Text Size `large`

#### Scenario: Missing Text Size defaults to medium

- **WHEN** `common-settings.json` exists with language / unit / country but no `textSize` key
- **AND** the App warm-reads preferences
- **THEN** the in-memory Text Size is `medium`

#### Scenario: Corrupt JSON soft-fails

- **WHEN** `common-settings.json` exists but is not valid JSON
- **THEN** the App applies Language `en-US`, Unit `Metric`, Country `US`, and Text Size `medium` defaults and continues without crashing
