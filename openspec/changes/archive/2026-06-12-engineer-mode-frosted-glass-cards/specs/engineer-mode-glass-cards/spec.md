...
### Requirement: Engineer Mode Reset and Save actions use FrostedGlassButton

The Engineer Mode **Reset to Default** and **Save as Common** action controls SHALL use `FrostedGlassButton` with `borderGradientCenter="top-left-bottom-right"` instead of `@mipmap` button backgrounds. Button labels, start drawables, enabled/disabled states, click handlers, and session baseline / save semantics MUST remain unchanged.

#### Scenario: Reset to Default glass button
- **WHEN** the operator views any Engineer Mode process tab
- **THEN** the Reset to Default control MUST render as `FrostedGlassButton` with `borderGradientCenter="top-left-bottom-right"`
- **AND** tapping it MUST restore the session baseline as before

#### Scenario: Save as Common glass button
- **WHEN** the operator views any Engineer Mode process tab
- **THEN** the Save as Common control MUST render as `FrostedGlassButton` with `borderGradientCenter="top-left-bottom-right"`
