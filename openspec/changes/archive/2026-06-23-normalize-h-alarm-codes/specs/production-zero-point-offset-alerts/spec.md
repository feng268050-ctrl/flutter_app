...
The alert SHALL use alarm code **H034**, SHALL persist a SERIOUS row in `warn_table` when the episode starts, and SHALL use the coded-alarm passive warn pipeline (`WarnCacheManager` + `DeviceDialogHandler` + `AutoDialogQueue`).
