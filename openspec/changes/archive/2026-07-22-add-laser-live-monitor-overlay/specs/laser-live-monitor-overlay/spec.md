...
The App SHALL provide a shared overlay body (`LaserLiveMonitorOverlayFragment` or equivalent) that displays (1) a local MediaMTX **PR1** live preview, (2) AI detection boxes drawn via `DetectionOverlayView` (or equivalent) over that preview, and (3) a compact machine-status gauges/tiles sidebar bound to existing device MemoryCache fields. The overlay MUST NOT start or stop the native stream-detect pipeline; it MUST only subscribe to published detect results (e.g. `StreamDetectResultBus`) while visible.
...
- **AND** the body MUST show PR1 preview and the gauges sidebar
...
### Requirement: Sidebar gauges keep dialog frosted glass chrome

The Live Monitor gauges/tiles sidebar SHALL continue to use dialog-variant frosted glass machine-status gauge/tile components (same visual contract as the prior More Monitor dialog gauges) with Modbus/status bindings functionally unchanged for the v1 field set.

#### Scenario: Sidebar gauges remain frosted dialog variant

- **WHEN** Live Monitor overlay displays the gauges sidebar
- **THEN** gauge/tile containers MUST use dialog-variant frosted glass chrome
- **AND** status indicators MUST continue to reflect `DeviceStatus` on/off bindings
