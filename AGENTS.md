...
- **Generic OS vs product vs hardware (do not conflate):**
  ...
  - **ARM / DT model** — **Device Tree** describes board wiring and probe; **Image** holds compiled drivers/Kconfig. **New board** → new DTB + OEM (+ optional `uboot_id`); **reuse Image**. **New SoC drivers** (e.g. RK3562 alongside RK3566) → extend `overlay/kernel/**/*.config`, **`FORCE_KERNEL_IMAGE=1 make build-kernel` once** for a **new universal Image**, then add DTBs to FIT inventory — **not** a per-board Image target. Canonical: [`docs/make-commands.md`](docs/make-commands.md) → **构建模型** → Kernel / 新板接入.
...
| `overlay/kernel/**` DTS / `.dtsi` only (no `*.config` fragment change) | `make apply-overlay`, then `make build-kernel`, `make upgrade` |
...
| `board/*.txt` LCD/MIPI platform defaults | `make apply-overlay`, `make build-rootfs`, `make upgrade` — **per-SKU runtime tables → OEM** |
...
- **Paths:** app → `app/<APP>/`（release bundle → `app/<APP>/build/bundle/release/`）；platform rootfs overlay → `overlay/board/rockchip/common/rootfs-overlay/`（**不含** App bundle）；SoC 薄层 → `overlay/board/rockchip/$CHIP/rootfs-overlay/`（当前线 `rk3566_rk3568`，如 Innohi MainServer）；Buildroot fragments → `overlay/buildroot/`。
...
| `overlay/board/rockchip/$CHIP/rootfs-overlay/` | SoC-thin extras (current line: `rk3566_rk3568`, e.g. Innohi MainServer) |
